...
module.exports = class SocketServer {
    ...
    */
    ...
    aulasRemotas (socket) {
        const {token, nombre, apellido, usuario} = socket.handshake.query
        if (!token || token === null || token === undefined) {
        ...
        try {
            const { id } = jwt.verify(token, process.env.SECRET)
            const conexion = new Conexion(socket.id, id, socket.handshake.time, nombre, apellido, usuario)
            ...
            console.log("No tiene autorización.", error)
            socket.disconnect()
        }
    }
